<template>
  <b-container id="stac-browser">
        ...
        <b-col md="12">
          ...
          <nav class="actions user">
            <b-button-group>
              ...
              <LanguageChooser
                :data="data"
                :currentLocale="localeFromVueX"
                :locales="supportedLocalesFromVueX"
    ...
    <footer>
      ...
      <ul
        v-if="Array.isArray(footerLinksFromVueX) && footerLinksFromVueX.length > 0"
        class="footer-links text-body-secondary">
        <li v-for="link in footerLinksFromVueX" :key="link.url">
          <a :href="link.url" target="_blank">{{
            $te(`footerLinks.${link.label}`) ? $t(`footerLinks.${link.label}`) : link.label
          }}</a>
        </li>
      </ul>
    ...
    root(root, oldRoot) {
      ...
      for (let key of canChange) {
        ...
        if (typeof value !== 'undefined') {
          this.$store.dispatch('config', { [key]: value }).catch(error => console.error(error));
        }
      }
    },
    data(data) {
