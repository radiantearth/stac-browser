    ...
    <header>
      <b-row class="site">
        <b-col md="12">
          ...
          <div class="title">
            <img :src="catalogImageFromVueX" class="logo">
            <h2>
              <StacLink v-if="root" :data="root" />
              <template v-else>{{ catalogTitle }}</template>
            </h2>
            <b-button v-if="root" size="sm" variant="outline-primary" id="popover-root-btn" :title="$t('info')">
              <b-icon-info-lg />
            </b-button>
          </div>
          <nav class="actions">
      ...
      <b-row class="page">
        <b-col md="12">
          ...
          <nav class="actions" v-if="back || collectionLink || parentLink">
...
import {
  ...
  BIconFlag, BIconFolderSymlink, BIconInfoLg, BIconList, BIconSearch,
