...
export default defineComponent({
  ...
  computed: {
    ...
    refererLink() {
      ...
      if (!this.allowExternalAccess && Utils.isExternalBrowserPath(referer)) {
        return null;
      }
      return referer;
    },
    ...
    dataLanguage: {
      ...
      async handler(locale) {
        ...
        if (this.data instanceof STAC) {
          ...
          else if (this.supportsConformance(languageConformance, this.data)) {
  ...
  async created() {
    ...
    this.$router.beforeEach((to, from) => {
      if (this.allowSelectCatalog || !this.allowExternalAccess) {
        return;
      }
