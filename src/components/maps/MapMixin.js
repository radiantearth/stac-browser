  ...
  methods: {
    ...
    createControls() {
      ...
      this.zoomControl = new ZoomControl({
        zoomInLabel: this.$i18n.t('mapping.zoom.in.label'),
        zoomOutLabel: this.$i18n.t('mapping.zoom.out.label'),
        zoomInTipLabel: this.$i18n.t('mapping.zoom.in.description'),
        zoomOutTipLabel: this.$i18n.t('mapping.zoom.out.description')
      });
      this.map.addControl(this.zoomControl);

      this.attributionControl = new AttributionControl({
        tipLabel: this.$i18n.t('mapping.attribution.description'),
        label: this.$i18n.t('mapping.attribution.label'),
        collapseLabel: this.$i18n.t('mapping.attribution.collapseLabel'),
      });
      this.map.addControl(this.attributionControl);

      this.fullScreenControl = new FullScreenControl({
        label: this.$i18n.t('fullscreen.showLabel'),
        labelActive: this.$i18n.t('fullscreen.exitLabel'),
        tipLabel: this.$i18n.t('fullscreen.show'),
      });
      this.fullScreenControl.on('enterfullscreen', () => {
        this.fullScreenControl.button_.title = this.$i18n.t('fullscreen.exit');
        this.isFullScreen = true;
      });
      this.fullScreenControl.on('leavefullscreen', () => {
        this.fullScreenControl.button_.title = this.$i18n.t('fullscreen.show');
