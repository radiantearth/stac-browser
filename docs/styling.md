...
For deployments where you don't want to rebuild from source (e.g. Docker images), STAC Browser supports a **runtime stylesheet**.
...
- Breakpoints, spacing scales, and border radii are compiled into the CSS and can't be changed at runtime. Heading font sizes are also build-time only, except the page title, which follows `--sb-h1-font-size`.
