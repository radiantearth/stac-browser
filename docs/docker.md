...
You can not, because `pathPrefix` is a build-only option.
Either serve STAC Browser from its own (sub)domain instead of a sub-path,
or build your own image, which is a single command:
