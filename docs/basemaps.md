...
In addition, you can add a dark mode variant of each basemap in `BASEMAPS` object. Just append `-dark` to the celestial body, e.g. `earth-dark` (all keys must be lowercase). The default version of `configureBasemap` detects and uses the dark basemaps automatically if provided in the `BASEMAPS` object.
